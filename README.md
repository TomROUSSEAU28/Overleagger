...
| `VERIFY_EMAIL`                                                      | `true` (with SMTP)              | `false`: new accounts don't confirm their e-mail with a code                                                                                                                                                                                                      |
...
| `CONTACT_EMAIL`                                                     | —                               | Where messages of the homepage contact form are e-mailed                                                                                                                                                                                                          |
| `SMTP_HOST` / `SMTP_PORT` / `SMTP_USER` / `SMTP_PASS` / `SMTP_FROM` | —                               | Outgoing e-mail (e.g. Brevo): the contact form, the codes that confirm a new account's e-mail and let people choose a new password; without it, messages are only on the Administration page, accounts are not confirmed, and passwords cannot be reset by e-mail |
